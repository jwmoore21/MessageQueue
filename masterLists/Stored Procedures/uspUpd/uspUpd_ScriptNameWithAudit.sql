/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspUpd_ScriptNameWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspUpd_ScriptNameWithAudit]
GO
*/

CREATE PROCEDURE [masterLists].[uspUpd_ScriptNameWithAudit]
...
================================================================================
Schema:      masterLists
Object:      uspUpd_ScriptNameWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Update proc ran: [masterLists].[uspUpd_ScriptNameWithAudit]' );
    ...
    SELECT
      ...
      , [ScriptName].[RowStatus]
      , [ScriptName].[CreatedBy]
      , [ScriptName].[ModifiedBy]
      , [ScriptName].[CreatedDate]
      , [ScriptName].[ModifiedDate]
      , [ScriptName].[RowGuid]
    FROM
      [masterLists].[ScriptName] AS ScriptName
    WHERE
    ...
    (
      ...
      AND [tables].[name] = 'AuditScriptName'
    );
    ...
    (
      ...
      AND OBJECT_NAME([systemObject].[parent_object_id]) = 'ScriptName'
    ...
    BEGIN
      ...
      /* Parent table has an update trigger enabled, and the audit table
         ...
         the auditing */
      UPDATE [masterLists].[ScriptName]
      SET
         [ScriptName].[RowStatus]      = [ChangeTableVar].[RowStatus]
        , [ScriptName].[CreatedBy]      = [ChangeTableVar].[CreatedBy]
        , [ScriptName].[ModifiedBy]     = [ChangeTableVar].[ModifiedBy]
        , [ScriptName].[CreatedDate]    = [ChangeTableVar].[CreatedDate]
        , [ScriptName].[ModifiedDate]   = [ChangeTableVar].[ModifiedDate]
        , [ScriptName].[RowGuid]        = [ChangeTableVar].[RowGuid]

      FROM #ChangeTableVar AS [ChangeTableVar]
        INNER JOIN [masterLists].[ScriptName] AS ScriptName
        ON [ScriptName].[ScriptName] = [ChangeTableVar].[ScriptName]
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        INSERT INTO [audit].[AuditScriptName]
        (
            [AuditId]
          , [AuditStatus]
          , [AuditAppUser]
    ...
    FROM
      [audit].[AuditScriptName] AS [AuditScriptName]
    WHERE
      [AuditScriptName].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
            [AuditScriptName].[ScriptName]
          , [AuditScriptName].[RowStatus]
          , [AuditScriptName].[CreatedBy]
          , [AuditScriptName].[ModifiedBy]
          , [AuditScriptName].[CreatedDate]
          , [AuditScriptName].[ModifiedDate]
          , [AuditScriptName].[RowGuid]
        FROM
          [audit].[AuditScriptName] AS [AuditScriptName]
        WHERE
          [AuditScriptName].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditScriptName].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ScriptName'), ROOT ('UpdatedRecords'), ELEMENTS XSINIL
            );
...
DECLARE @ScriptName VARCHAR;
SET @ScriptName = FILL IN;

EXEC masterLists.uspUpd_ScriptNameWithAudit
  @ScriptName = @ScriptName,
  @modified_by    = -73,
  @ReturnOutput   = 1,
  @ReturnAsTable  = 1,
...
SELECT AuditScriptName.*
FROM   audit.AuditScriptName AS AuditScriptName
WHERE
  AuditScriptName.ScriptName = @ScriptName
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspUpd_ScriptNameWithAudit';
GO