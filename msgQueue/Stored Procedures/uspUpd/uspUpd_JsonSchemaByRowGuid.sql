/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[msgQueue].[uspUpd_JsonSchemaByRowGuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [msgQueue].[uspUpd_JsonSchemaByRowGuid]
GO
*/

CREATE PROCEDURE [msgQueue].[uspUpd_JsonSchemaByRowGuid]
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Update proc ran: [msgQueue].[uspUpd_JsonSchemaByRowGuid]' );
    ...
    END
      ...
      FROM #ChangeTableVar AS [ChangeTableVar]
        ...
        ON [JsonSchema].[RowGuid] = [ChangeTableVar].[RowGuid]
      ;
...
EXEC msgQueue.uspUpd_JsonSchemaByRowGuid
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspUpd_JsonSchemaByRowGuid';
GO