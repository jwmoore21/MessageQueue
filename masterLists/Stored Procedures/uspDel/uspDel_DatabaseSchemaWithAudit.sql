/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspDel_DatabaseSchemaWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspDel_DatabaseSchemaWithAudit]
GO
*/

CREATE PROCEDURE [masterLists].[uspDel_DatabaseSchemaWithAudit]
...
================================================================================
Schema:      masterLists
Object:      uspDel_DatabaseSchemaWithAudit
...
BEGIN
  ...
  */
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: [masterLists].[uspDel_DatabaseSchemaWithAudit]' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'DatabaseSchema'
    ...
    BEGIN
      ...
      /* Parent table has an update trigger enabled, and the audit table
         ...
         the auditing */
      DELETE FROM [masterLists].[DatabaseSchema]

      WHERE
        [DatabaseSchema].[DatabaseSchema] = @_DatabaseSchema
      ;
    ...
    FROM
      [audit].[AuditDatabaseSchema] AS [AuditDatabaseSchema]
    WHERE
      [AuditDatabaseSchema].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
            [AuditDatabaseSchema].[DatabaseSchema]
          , [AuditDatabaseSchema].[RowStatus]
          , [AuditDatabaseSchema].[CreatedBy]
          , [AuditDatabaseSchema].[ModifiedBy]
          , [AuditDatabaseSchema].[CreatedDate]
          , [AuditDatabaseSchema].[ModifiedDate]
          , [AuditDatabaseSchema].[RowGuid]
        FROM
          [audit].[AuditDatabaseSchema] AS [AuditDatabaseSchema]
        WHERE
          [AuditDatabaseSchema].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditDatabaseSchema].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('DatabaseSchema'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC masterLists.uspDel_DatabaseSchemaWithAudit
...
WHERE
  AuditDatabaseSchema.AuditBatchId = @AuditBatchId
;
*/
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_DatabaseSchemaWithAudit';
GO