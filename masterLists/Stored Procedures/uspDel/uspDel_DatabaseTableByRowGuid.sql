/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspDel_DatabaseTableByRowGuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspDel_DatabaseTableByRowGuid]
GO
*/

CREATE PROCEDURE [masterLists].[uspDel_DatabaseTableByRowGuid]
...
================================================================================
Schema:      masterLists
Object:      uspDel_DatabaseTableByRowGuid
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: [masterLists].[uspDel_DatabaseTableByRowGuid]' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'DatabaseTable'
    ...
    BEGIN
      ...
      /* Parent table has an update trigger enabled, and the audit table
         ...
         the auditing */
      DELETE FROM [masterLists].[DatabaseTable]

      WHERE
        [DatabaseTable].[RowGuid] = @_RowGuid
      ;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditDatabaseTable].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('DatabaseTable'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC masterLists.uspDel_DatabaseTableByRowGuid
  @RowGuid = @RowGuid,
...
WHERE
  AuditDatabaseTable.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_DatabaseTableByRowGuid';
GO