/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspDel_EventTypeByRowGuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspDel_EventTypeByRowGuid]
GO
*/

CREATE PROCEDURE [masterLists].[uspDel_EventTypeByRowGuid]
...
================================================================================
Schema:      masterLists
Object:      uspDel_EventTypeByRowGuid
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: [masterLists].[uspDel_EventTypeByRowGuid]' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'EventType'
    ...
    BEGIN
      ...
      /* Parent table has an update trigger enabled, and the audit table
         ...
         the auditing */
      DELETE FROM [masterLists].[EventType]

      WHERE
        [EventType].[RowGuid] = @_RowGuid
      ;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditEventType].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('EventType'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC masterLists.uspDel_EventTypeByRowGuid
  @RowGuid = @RowGuid,
...
WHERE
  AuditEventType.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_EventTypeByRowGuid';
GO