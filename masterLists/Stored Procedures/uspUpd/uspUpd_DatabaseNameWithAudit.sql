/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspUpd_DatabaseNameWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspUpd_DatabaseNameWithAudit]
GO
*/

CREATE PROCEDURE [masterLists].[uspUpd_DatabaseNameWithAudit]
...
================================================================================
Schema:      masterLists
Object:      uspUpd_DatabaseNameWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Update proc ran: [masterLists].[uspUpd_DatabaseNameWithAudit]' );
    ...
    (
      ...
      AND OBJECT_NAME([systemObject].[parent_object_id]) = 'DatabaseName'
    ...
    BEGIN
      ...
      /* Parent table has an update trigger enabled, and the audit table
         ...
         the auditing */
      UPDATE [masterLists].[DatabaseName]
      SET
         [DatabaseName].[RowStatus]      = [ChangeTableVar].[RowStatus]
        , [DatabaseName].[CreatedBy]      = [ChangeTableVar].[CreatedBy]
        , [DatabaseName].[ModifiedBy]     = [ChangeTableVar].[ModifiedBy]
        , [DatabaseName].[CreatedDate]    = [ChangeTableVar].[CreatedDate]
        , [DatabaseName].[ModifiedDate]   = [ChangeTableVar].[ModifiedDate]
        , [DatabaseName].[RowGuid]        = [ChangeTableVar].[RowGuid]

      FROM #ChangeTableVar AS [ChangeTableVar]
        INNER JOIN [masterLists].[DatabaseName] AS DatabaseName
        ON [DatabaseName].[DatabaseName] = [ChangeTableVar].[DatabaseName]
      ;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditDatabaseName].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('DatabaseName'), ROOT ('UpdatedRecords'), ELEMENTS XSINIL
            );
...
EXEC masterLists.uspUpd_DatabaseNameWithAudit
  @DatabaseName = @DatabaseName,
  @modified_by    = -73,
  @ReturnOutput   = 1,
  @ReturnAsTable  = 1,
...
WHERE
  AuditDatabaseName.DatabaseName = @DatabaseName
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspUpd_DatabaseNameWithAudit';
GO