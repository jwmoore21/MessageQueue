/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspDel_HttpVerbByRowGuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspDel_HttpVerbByRowGuid]
GO
*/

CREATE PROCEDURE [masterLists].[uspDel_HttpVerbByRowGuid]
...
================================================================================
Schema:      masterLists
Object:      uspDel_HttpVerbByRowGuid
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: [masterLists].[uspDel_HttpVerbByRowGuid]' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'HttpVerb'
    ...
    BEGIN
      ...
      /* Parent table has an update trigger enabled, and the audit table
         ...
         the auditing */
      DELETE FROM [masterLists].[HttpVerb]

      WHERE
        [HttpVerb].[RowGuid] = @_RowGuid
      ;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditHttpVerb].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('HttpVerb'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC masterLists.uspDel_HttpVerbByRowGuid
  @RowGuid = @RowGuid,
...
WHERE
  AuditHttpVerb.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_HttpVerbByRowGuid';
GO