/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[msgQueue].[TRG__MessageQueue__after_upd]'))
DROP TRIGGER [msgQueue].[TRG__MessageQueue__after_upd]
GO
*/
CREATE TRIGGER [msgQueue].[TRG__MessageQueue__after_upd] ON [msgQueue].[MessageQueue]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      msgQueue
Object:      TRG__MessageQueue__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__MessageQueue__after_upd';
    /* ========================================================================= */
