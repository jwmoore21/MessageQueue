/*
IF  EXISTS (SELECT * FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspIns_Datasource]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspIns_Datasource]
GO
*/

CREATE PROCEDURE [masterLists].[uspIns_Datasource]
  /* Non-identity / guid Primary Key(s) */
  @NewDatasource  VARCHAR(255)      ,
  /* Foreign Key(s) */
  @RowStatus      TINYINT           ,
  /* General Column Data */
  ...
  @Datasource     VARCHAR(255) = NULL OUTPUT,
  /* XML Results Output */
...
BEGIN
  ...
  BEGIN TRY
    ...
    (
        [INSERTED_Datasource] VARCHAR(255) NOT NULL
    );

    /* ========================================================================= */
    /* Insert our record */
    INSERT INTO [masterLists].[Datasource]
    (
         [Datasource]
       , [RowStatus]
       , [CreatedBy]
    ...
    BEGIN
      ...
      SELECT
        @Datasource = [OutputTableVar].[INSERTED_Datasource]
      FROM #OutputTableVar AS OutputTableVar
        ...
        BEGIN TRY
          ...
          SELECT
            ...
            (
              ...
              WHERE
                [Datasource].[Datasource] = @Datasource
              FOR XML PATH ('Datasource'), ROOT ('InsertedRecords'), ELEMENTS XSINIL
            );
...
SET @Datasource      = [dbo].[ufn_LoremIpsumString](255, 0);

EXEC masterLists.uspIns_Datasource
  /* Non-identity / guid Primary Key(s) */
  @Datasource      = @Datasource,  /* Foreign Key(s) */
  @RowStatus       = @RowStatus,
  /* General Column Data */
  /* App User Id / Modified By Id */
  @ModifiedBy      = NULL,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspIns_Datasource';
GO