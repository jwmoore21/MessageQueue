/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspDel_DatasourceByRowGuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspDel_DatasourceByRowGuid]
GO
*/

CREATE PROCEDURE [masterLists].[uspDel_DatasourceByRowGuid]
...
================================================================================
Schema:      masterLists
Object:      uspDel_DatasourceByRowGuid
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: [masterLists].[uspDel_DatasourceByRowGuid]' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'Datasource'
    ...
    BEGIN
      ...
      /* Parent table has an update trigger enabled, and the audit table
         ...
         the auditing */
      DELETE FROM [masterLists].[Datasource]

      WHERE
        [Datasource].[RowGuid] = @_RowGuid
      ;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditDatasource].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('Datasource'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC masterLists.uspDel_DatasourceByRowGuid
  @RowGuid = @RowGuid,
...
WHERE
  AuditDatasource.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_DatasourceByRowGuid';
GO