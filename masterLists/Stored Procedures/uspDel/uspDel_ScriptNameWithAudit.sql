/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspDel_ScriptNameWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspDel_ScriptNameWithAudit]
GO
*/

CREATE PROCEDURE [masterLists].[uspDel_ScriptNameWithAudit]
...
================================================================================
Schema:      masterLists
Object:      uspDel_ScriptNameWithAudit
...
BEGIN
  ...
  */
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: [masterLists].[uspDel_ScriptNameWithAudit]' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ScriptName'
    ...
    BEGIN
      ...
      /* Parent table has an update trigger enabled, and the audit table
         ...
         the auditing */
      DELETE FROM [masterLists].[ScriptName]
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditScriptName].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ScriptName'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC masterLists.uspDel_ScriptNameWithAudit
...
WHERE
  AuditScriptName.AuditBatchId = @AuditBatchId
;
*/
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ScriptNameWithAudit';
GO