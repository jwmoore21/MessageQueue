/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspDel_SubHostNameWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspDel_SubHostNameWithAudit]
GO
*/

CREATE PROCEDURE [masterLists].[uspDel_SubHostNameWithAudit]
...
================================================================================
Schema:      masterLists
Object:      uspDel_SubHostNameWithAudit
...
BEGIN
  ...
  */
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: [masterLists].[uspDel_SubHostNameWithAudit]' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'SubHostName'
    ...
    BEGIN
      ...
      /* Parent table has an update trigger enabled, and the audit table
         ...
         the auditing */
      DELETE FROM [masterLists].[SubHostName]

      WHERE
        [SubHostName].[SubHostName] = @_SubHostName
      ;
    ...
    FROM
      [audit].[AuditSubHostName] AS [AuditSubHostName]
    WHERE
      [AuditSubHostName].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
            [AuditSubHostName].[SubHostName]
          , [AuditSubHostName].[RowStatus]
          , [AuditSubHostName].[CreatedBy]
          , [AuditSubHostName].[ModifiedBy]
          , [AuditSubHostName].[CreatedDate]
          , [AuditSubHostName].[ModifiedDate]
          , [AuditSubHostName].[RowGuid]
        FROM
          [audit].[AuditSubHostName] AS [AuditSubHostName]
        WHERE
          [AuditSubHostName].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditSubHostName].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('SubHostName'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC masterLists.uspDel_SubHostNameWithAudit
...
WHERE
  AuditSubHostName.AuditBatchId = @AuditBatchId
;
*/
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_SubHostNameWithAudit';
GO