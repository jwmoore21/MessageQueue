/*
IF  EXISTS (SELECT * FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspIns_ScriptName]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspIns_ScriptName]
GO
*/

CREATE PROCEDURE [masterLists].[uspIns_ScriptName]
  /* Non-identity / guid Primary Key(s) */
  @NewScriptName  VARCHAR(128)      ,
  /* Foreign Key(s) */
  @RowStatus      TINYINT           ,
  /* General Column Data */
  ...
  @ScriptName     VARCHAR(128) = NULL OUTPUT,
  /* XML Results Output */
...
BEGIN
  ...
  BEGIN TRY
    ...
    (
        [INSERTED_ScriptName] VARCHAR(128) NOT NULL
    );

    /* ========================================================================= */
    /* Insert our record */
    INSERT INTO [masterLists].[ScriptName]
    (
         [ScriptName]
       , [RowStatus]
       , [CreatedBy]
    ...
    BEGIN
      ...
      SELECT
        @ScriptName = [OutputTableVar].[INSERTED_ScriptName]
      FROM #OutputTableVar AS OutputTableVar
        ...
        BEGIN TRY
          ...
          SELECT
            ...
            (
              ...
              WHERE
                [ScriptName].[ScriptName] = @ScriptName
              FOR XML PATH ('ScriptName'), ROOT ('InsertedRecords'), ELEMENTS XSINIL
            );
...
SET @ScriptName      = [dbo].[ufn_LoremIpsumString](128, 0);

EXEC masterLists.uspIns_ScriptName
  /* Non-identity / guid Primary Key(s) */
  @ScriptName      = @ScriptName,  /* Foreign Key(s) */
  @RowStatus       = @RowStatus,
  /* General Column Data */
  /* App User Id / Modified By Id */
  @ModifiedBy      = NULL,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspIns_ScriptName';
GO