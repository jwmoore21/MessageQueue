/*
IF  EXISTS (SELECT * FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspIns_EventType]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspIns_EventType]
GO
*/

CREATE PROCEDURE [masterLists].[uspIns_EventType]
  /* Non-identity / guid Primary Key(s) */
  @NewEventType   VARCHAR(255)      ,
  /* Foreign Key(s) */
  @RowStatus      TINYINT           ,
  /* General Column Data */
  ...
  @EventType      VARCHAR(255) = NULL OUTPUT,
  /* XML Results Output */
...
BEGIN
  ...
  BEGIN TRY
    ...
    (
        [INSERTED_EventType] VARCHAR(255) NOT NULL
    );

    /* ========================================================================= */
    /* Insert our record */
    INSERT INTO [masterLists].[EventType]
    (
         [EventType]
       , [RowStatus]
       , [CreatedBy]
    ...
    BEGIN
      ...
      SELECT
        @EventType = [OutputTableVar].[INSERTED_EventType]
      FROM #OutputTableVar AS OutputTableVar
        ...
        BEGIN TRY
          ...
          SELECT
            ...
            (
              ...
              WHERE
                [EventType].[EventType] = @EventType
              FOR XML PATH ('EventType'), ROOT ('InsertedRecords'), ELEMENTS XSINIL
            );
...
SET @EventType       = [dbo].[ufn_LoremIpsumString](255, 0);

EXEC masterLists.uspIns_EventType
  /* Non-identity / guid Primary Key(s) */
  @EventType       = @EventType,  /* Foreign Key(s) */
  @RowStatus       = @RowStatus,
  /* General Column Data */
  /* App User Id / Modified By Id */
  @ModifiedBy      = NULL,
