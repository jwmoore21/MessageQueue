/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspUpd_DatabaseSchemaWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspUpd_DatabaseSchemaWithAudit]
GO
*/

CREATE PROCEDURE [masterLists].[uspUpd_DatabaseSchemaWithAudit]
...
================================================================================
Schema:      masterLists
Object:      uspUpd_DatabaseSchemaWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Update proc ran: [masterLists].[uspUpd_DatabaseSchemaWithAudit]' );
    ...
    BEGIN
      ...
      FROM #ChangeTableVar AS [ChangeTableVar]
        ...
        ON [DatabaseSchema].[DatabaseSchema] = [ChangeTableVar].[DatabaseSchema]
      ;
...
EXEC masterLists.uspUpd_DatabaseSchemaWithAudit
  @DatabaseSchema = @DatabaseSchema,
  @modified_by    = -73,
  @ReturnOutput   = 1,
  @ReturnAsTable  = 1,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspUpd_DatabaseSchemaWithAudit';
GO