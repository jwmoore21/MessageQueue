/*
IF  EXISTS (SELECT * FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspIns_DomainName]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspIns_DomainName]
GO
*/

CREATE PROCEDURE [masterLists].[uspIns_DomainName]
...
BEGIN
  ...
  BEGIN TRY
    ...
    /* ========================================================================= */
    /* Insert our record */
    INSERT INTO [masterLists].[DomainName]
    (
         [DomainName]
       , [RowStatus]
       , [CreatedBy]
        ...
        BEGIN TRY
          ...
          SELECT
            ...
            (
              ...
              WHERE
                [DomainName].[DomainName] = @DomainName
              FOR XML PATH ('DomainName'), ROOT ('InsertedRecords'), ELEMENTS XSINIL
            );
...
EXEC masterLists.uspIns_DomainName
  /* Non-identity / guid Primary Key(s) */
  @DomainName      = @DomainName,  /* Foreign Key(s) */
  @RowStatus       = @RowStatus,
  /* General Column Data */
  /* App User Id / Modified By Id */
  @ModifiedBy      = NULL,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspIns_DomainName';
GO