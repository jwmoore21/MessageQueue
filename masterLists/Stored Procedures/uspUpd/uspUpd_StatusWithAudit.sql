/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[masterLists].[uspUpd_StatusWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [masterLists].[uspUpd_StatusWithAudit]
GO
*/

CREATE PROCEDURE [masterLists].[uspUpd_StatusWithAudit]
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Update proc ran: [masterLists].[uspUpd_StatusWithAudit]' );
    ...
    UPDATE #ChangeTableVar
      SET
        ...
        , [bSystemOnly] =
          CASE
            WHEN @_bSystemOnly IS NOT NULL THEN @_bSystemOnly
            WHEN LEN( @_bSystemOnly ) > 0 THEN [bSystemOnly]
          END
        , [StatusName] =
          CASE
            WHEN @_StatusName IS NOT NULL THEN @_StatusName
            WHEN LEN( @_StatusName ) > 0 THEN [StatusName]
          END
        , [StatusDescription] =
          CASE
            WHEN @_StatusDescription IS NOT NULL THEN @_StatusDescription
            WHEN LEN( @_StatusDescription ) > 0 THEN [StatusDescription]
            ELSE NULL
          END
    ...
    (
      ...
      AND OBJECT_NAME([systemObject].[parent_object_id]) = 'Status'
    ...
    BEGIN
      ...
      /* Parent table has an update trigger enabled, and the audit table
         ...
         the auditing */
      UPDATE [masterLists].[Status]
      SET
         [Status].[bSystemOnly]       = [ChangeTableVar].[bSystemOnly]
        , [Status].[CreatedBy]         = [ChangeTableVar].[CreatedBy]
        , [Status].[ModifiedBy]        = [ChangeTableVar].[ModifiedBy]
        , [Status].[CreatedDate]       = [ChangeTableVar].[CreatedDate]
        , [Status].[ModifiedDate]      = [ChangeTableVar].[ModifiedDate]
        , [Status].[RowGuid]           = [ChangeTableVar].[RowGuid]
        , [Status].[StatusName]        = [ChangeTableVar].[StatusName]
        , [Status].[StatusDescription] = [ChangeTableVar].[StatusDescription]

      FROM #ChangeTableVar AS [ChangeTableVar]
        INNER JOIN [masterLists].[Status] AS Status
        ON [Status].[RowStatus] = [ChangeTableVar].[RowStatus]
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
          ...
          , [ChangeTableVar].[StatusName]
          , [ChangeTableVar].[StatusDescription]
    ...
    FROM
      [audit].[AuditStatus] AS [AuditStatus]
    WHERE
      [AuditStatus].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
            [AuditStatus].[RowStatus]
          , [AuditStatus].[bSystemOnly]
          , [AuditStatus].[CreatedBy]
          , [AuditStatus].[ModifiedBy]
          , [AuditStatus].[CreatedDate]
          , [AuditStatus].[ModifiedDate]
          , [AuditStatus].[RowGuid]
          , [AuditStatus].[StatusName]
          , [AuditStatus].[StatusDescription]
        FROM
          [audit].[AuditStatus] AS [AuditStatus]
        WHERE
          [AuditStatus].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditStatus].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('Status'), ROOT ('UpdatedRecords'), ELEMENTS XSINIL
            );
...
SET @RowStatus = FILL IN;

EXEC masterLists.uspUpd_StatusWithAudit
  @RowStatus = @RowStatus,
  @modified_by    = -73,
  @ReturnOutput   = 1,
  @ReturnAsTable  = 1,
...
SELECT AuditStatus.*
FROM   audit.AuditStatus AS AuditStatus
WHERE
  AuditStatus.RowStatus = @RowStatus
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspUpd_StatusWithAudit';
GO